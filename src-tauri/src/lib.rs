...
/// Отметка времени в единственном принятом в базе виде: UTC, миллисекунды, «Z».
///
/// Раньше здесь был `to_rfc3339()`, а он пишет наносекунды и смещение «+00:00».
/// В базе при этом лежали и записи из старой версии — в виде «…Z» и вовсе одной
/// датой. Три формата в одной колонке сравниваются между собой неверно: для
/// одного и того же момента «…000Z» и «…000000000+00:00» различаются уже на
/// двадцать третьем символе, где «Z» больше цифры, и запись из старой базы
/// оказывается новее записи из новой. Сортировка журнала по времени на этом
/// незаметно врала.
